#!/usr/bin/env python3
"""
Burst FIFO Analysis - WINDOW FEATURES (v9)

Cara pakai:
  python window_burst9.py <input.md> <start> <end>
  python window_burst9.py data.md 10 2000 --format raw
  python window_burst9.py data.md 10 2000 --format all
...
Kenapa v9 dibuat:
  ...
  - v9 menulis baris raw setiap satu window_size SELESAI dihitung,
    langsung ke file (CSV) lalu langsung dibuang dari memori.
    Peak RAM raw jadi hanya sebesar 1 window_size saja, bukan akumulasi
    dari 10 sampai 2000.

Catatan:
  - Raw window hanya mendeskripsikan isi window itu sendiri.
  - Tidak ada next_gr, next_cat, target_low, target_ge_5, dll.
  - median_gr ditambahkan sebagai statistik robust untuk heavy-tail.
  - density = window_size / duration_min.
  - duration_ratio sengaja TIDAK disimpan di raw; hitung saat analisis
    sebagai duration_min / median duration untuk window_size yang sama.
...
class SlidingMedian:
    """Median sliding-window O(log n) dengan dua heap + lazy deletion."""
    ...
    def _prune_low(self):
        while self.low:
            value = -self.low[0]
            count = self.delayed.get(value, 0)
            if count <= 0:
                break
    ...
    def remove(self, value):
        self.delayed[value] = self.delayed.get(value, 0) + 1
...
def build_windows_for_size(rounds, window_size):
    ...
    # Median sliding window. Median dihitung dari isi window sendiri;
    # tidak ada next_gr/target_* atau informasi ronde berikutnya.
    median_tracker = SlidingMedian()
    valid_window_values = 0
    ...
    for i in range(n_rounds):
        ...
        # Masukkan ronde terbaru ke tracker median.
        if value is not None:
        ...
        # Keluarkan ronde yang sudah berada di luar window.
        outgoing = gr[window_start - 1] if window_start > 0 else None
        if outgoing is not None:
            median_tracker.remove(outgoing)
        ...
        row = {
            ...
            # Median lebih robust daripada mean terhadap heavy-tail.
            "median_gr": (
                round(median_tracker.median(), 4)
                if valid_count
                else None
            ),

            # Ronde per menit dalam window ini.
            "density": (
                round(window_size / (duration / 60), 6)
                if duration > 0
                else None
            ),
...
# ============================================================
# ANALYSIS FEATURE: DURATION RATIO
# ============================================================

def add_duration_ratio(windows):
    """
    Tambahkan duration_ratio untuk analisis in-memory saja.

    duration_ratio = duration_min / median(duration_min)
    pada window_size yang sama.

    Sengaja tidak dimasukkan ke raw_fieldnames(), karena nilai ini
    bergantung pada baseline median seluruh window_size dan bukan
    deskripsi intrinsik satu window.
    """
    if not windows:
        return windows
    ...
    if baseline <= 0:
        for w in windows:
            w["duration_ratio"] = None
    else:
        for w in windows:
            w["duration_ratio"] = round(
                w["duration_min"] / baseline,
                6
            )
...
# ============================================================
# RAW OUTPUT (STREAMING - v9)
# ============================================================

def raw_fieldnames():
    """Daftar kolom raw, tetap/statis (tidak butuh pandas)."""

    # Hanya fitur intrinsik window; tidak ada target/next-round fields.
    base_cols = [
        "window_size",
        "window_no",
        "window_start",
...
Contoh pemakaian:
  ...
  python window_burst9.py data.md 10 10 --format raw

  python window_burst9.py data.md 10 20 --format raw
